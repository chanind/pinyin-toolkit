...
class UnicodeSchemaTest(TestBase):
    ...
    def test_reflect(self):
        ...
        self.assertTrue(tt1.select(order_by=desc('méil')).execute().fetchall() ==
                     [(2, 7), (1, 5)])
        self.assertTrue(tt2.select(order_by=desc('méil')).execute().fetchall() ==
                     [(2, 2), (1, 1)])
        self.assertTrue(tt3.select(order_by=desc('\u6e2c\u8a66_id')).
                     execute().fetchall() ==
                     [(2, 7, 2, 2), (1, 5, 1, 1)])
        meta.drop_all()
        metadata.create_all()
