...
class ZooMarkTest(TestBase):
    ...
    def test_baseline_1a_populate(self):
        ...
        Zoo.insert(inline=True).execute(Name='Montr\xe9al Biod\xf4me',
                Founded=datetime.date(1992, 6, 19),
                Opens=datetime.time(9, 0, 0), Admission=11.75)
        seaworld = Zoo.insert().execute(Name='Sea_World',
        ...
        leopardid = Animal.insert().execute(Species='Leopard',
        ...
        lion = Animal.insert().execute(Species='Lion',
                ZooID=wap).inserted_primary_key[0]
        Animal.insert().execute(Species='Slug', Legs=1, Lifespan=.75)
        tiger = Animal.insert().execute(Species='Tiger',
                ZooID=sdz).inserted_primary_key[0]

        # Override Legs.default with itself just to make sure it works.

        Animal.insert(inline=True).execute(Species='Bear', Legs=4)
        Animal.insert(inline=True).execute(Species='Ostrich', Legs=2,
                Lifespan=103.2)
        Animal.insert(inline=True).execute(Species='Centipede',
                Legs=100)
        emp = Animal.insert().execute(Species='Emperor Penguin',
                Legs=2, ZooID=seaworld).inserted_primary_key[0]
        adelie = Animal.insert().execute(Species='Adelie Penguin',
                Legs=2, ZooID=seaworld).inserted_primary_key[0]
        Animal.insert(inline=True).execute(Species='Millipede',
                Legs=1000000, ZooID=sdz)

        # Add a mother and child to test relationships

        bai_yun = Animal.insert().execute(Species='Ape',
                Name='Bai Yun', Legs=2).inserted_primary_key[0]
        Animal.insert(inline=True).execute(Species='Ape',
    ...
    def test_baseline_6_editing(self):
        ...
        for x in range(ITERATIONS):
            ...
            Zoo.update(Zoo.c.ID == SDZ['ID'
                       ]).execute(Name='The San Diego Zoo',
            ...
            SDZ = Zoo.select(Zoo.c.Name == 'The San Diego Zoo'
                             ).execute().first()
